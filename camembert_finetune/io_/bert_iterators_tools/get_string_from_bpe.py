from camembert_finetune.env.imports import OrderedDict, torch, pdb, re
...
from camembert_finetune.io_.bert_iterators_tools.string_processing import from_bpe_token_to_str
from camembert_finetune.trainer.tools.epoch_run_fine_tuning_tools import get_task_name_based_on_logit_label
...
def get_prediction(logits_dic, topk):
    ...
    for logit_label, logits in logits_dic.items():
        ...
        if logit_label == "parsing-types":
            ...
            predictions_topk_dic[logit_label] = torch.argmax(logits, dim=-1).unsqueeze(-1)
...
def get_bpe_string(predictions_topk_dic,
    ...
    for task_label in predictions_topk_dic:
        ...
        source_preprocessed[input_label] = from_bpe_token_to_str(input_tokens_tensor, topk, tokenizer=tokenizer, label_dictionary=task_to_label_dictionary[task_label], pred_mode=False, task=task,
                                                                 #null_token_index=null_token_index, null_str=null_str,
                                                                 get_bpe_string=True, verbose=verbose)
...
def get_detokenized_str(source_preprocessed_dict, input_alignement_with_raw, label_dic, predict_dic,
                        mask_str, end_token,
                        #null_str, remove_mask_str_prediction,
                        task_settings, #batch=None,
                        flag_is_first_token=False, flag_word_piece_token="##",cumulate_shift_sub_word=None):
