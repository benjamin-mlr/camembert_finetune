from camembert_finetune.env.imports import sys, os, codecs,  re, pdb
from camembert_finetune.io_.dat.ioutils import DependencyInstance, Sentence, SentenceWordPieced
from camembert_finetune.io_.dat.constants import DIGIT_RE, MAX_CHAR_LENGTH, MAX_WORDPIECES_LENGTH, NULL_STR,  NUM_CHAR_PAD, ROOT, ROOT_CHAR, ROOT_POS, ROOT_TYPE, PAD, END_CHAR, END_POS, END_TYPE, END, ROOT_HEADS_INDEX, END_HEADS_INDEX
from camembert_finetune.io_.logger import printing
from camembert_finetune.io_.bert_iterators_tools.alignement import align_bpe
...
from camembert_finetune.model.settings import TASKS_PARAMETER, AVAILABLE_INPUTS
...
class CoNLLReader(object):
  ...
  def __init__(self, file_path, word_dictionary,
    ...
    if bert_tokenizer is not None:
      printing("INFO Reader : will provide BERT bpe tokens", verbose=1, verbose_level=2)
    ...
    for tokens in lines:
        ...
        if '-' in tokens[0] and get_mwe:
          ...
          if self.bert_tokenizer is not None:
            ...
            try:
              ...
              mwe = self.bert_tokenizer.tokenize(tokens[1], )
          ...
          printing("WARNING : (reader) skipping {} line because . found inside index", var=[tokens], verbose=verbose,
                   verbose_level=2)
      ...
      if len(tokens) < 10:
        ...
        if os.environ.get("EXPERIENCE") is not None:
          print("WARNING : WRITING corrupted gold data in {} ".format(
            os.path.join(os.environ["EXPERIENCE"], "logs/catching_errors.txt")))
          open(os.path.join(os.environ["EXPERIENCE"], "logs/catching_errors.txt"), "a").write(
      ...
      if self.bert_tokenizer is not None:
        try:
          ...
          bpe_word = self.bert_tokenizer.tokenize(_word, add_special_tokens=False)
        ...
        try:
          ...
          bpe_lemma = self.bert_tokenizer.tokenize(tokens[2], add_special_tokens=False)
        ...
        if eval(tokens[0]) > id_stop_mwe:
          ...
          try:
            ...
            bert_pre_tokens = self.bert_tokenizer.tokenize(tokens[1], add_special_tokens=False)
        ...
        # to align bpes of raw sentence and  bpes of tokenized sequence
          ...
          if eval(tokens[0]) == id_stop_mwe:
            ...
            try:
              ...
              mwe_splits_save = self.bert_tokenizer.tokenize(" ".join(mwe_splits_save), add_special_tokens=False)
            ...
            word_piece_words_with_raw_aligned_index, n_masks_to_add_in_raw_label = align_bpe(n_masks_to_add_in_raw,
                                                                                             word_piece_raw_tokens_aligned,
                                                                                             word_piece_raw_tokens_aligned_index,
                                                                                             word_piece_words_with_raw_aligned,
                                                                                             word_piece_words_with_raw_aligned_index,
                                                                                             bert_tokenizer=self.bert_tokenizer,
                                                                                             src_token_len=len(mwe),
                                                                                             index_src=index_mwe,
                                                                                             n_masks_to_add=n_masks_to_add_in_raw_label,
                                                                                             mask_token=self.bert_tokenizer.mask_token,
                                                                                             index_target=id_stop_mwe,
                                                                                             mode="dummy",
                                                                                             verbose=verbose)
            if verbose == "reader":
              printing(
                "SRC token unsegmented : {} bpe noise : {} \nTARGET word segmented : {} bpe : {} \n IS_MWE (all vector) {}",
                var=[mwe_token, mwe, mwe_splits_save_as_token, mwe_splits_save, is_mwe], verbose_level="reader",
                verbose=verbose)
      ...
      if "parsing" in tasks:
        ...
        type = tokens[7]
        if run_mode == "train":
          assert head != "_", "ERROR : head not found for line {} while tasks is {} on data {} ".format(lines, tasks,
                                                                                                        self.file_path)
          assert type != "_", "ERROR : type not found for line {} while tasks is {} on data {} ".format(lines, tasks,
                                                                                                        self.file_path)
        types.append(type)
