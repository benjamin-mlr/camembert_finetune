from camembert.downstream.finetune.env.imports import pdb, OrderedDict, json, torch, re, np, SummaryWriter, time, os, sys
from camembert.downstream.finetune.io_.logger import printing
from camembert.downstream.finetune.env.gpu_tools.gpu_info import use_gpu_, printout_allocated_gpu_memory


from camembert.downstream.finetune.env.flags import REPORT_FLAG_DIR_STR
from camembert.downstream.finetune.io_.manage_dirs.make_dirs import setup_repoting_location
from camembert.downstream.finetune.model.optimization.get_optmizers import apply_fine_tuning_strategy
from camembert.downstream.finetune.io_.runs_tracker.google_sheet_report import update_status

from camembert.downstream.finetune.io_.data_iterator import readers_load, data_gen_multi_task_sampling_batch
from camembert.downstream.finetune.io_.dat import conllu_data

from camembert.downstream.finetune.io_.build_files_shard import build_shard
from camembert.downstream.finetune.io_.get_new_batcher import get_new_shard
...
from camembert.downstream.finetune.trainer.epoch_run import epoch_run
from camembert.downstream.finetune.model.architecture.get_model import get_model_multi_task_bert
from camembert.downstream.finetune.io_.report.report_tools import write_args, get_hyperparameters_dict, get_dataset_label, get_name_model_id_with_extra_name

from camembert.downstream.finetune.env.dir.project_directories import CHECKPOINT_BERT_DIR
from camembert.downstream.finetune.env.vars import N_SENT_MAX_CONLL_PER_SHARD
...
from camembert.downstream.finetune.transformers.transformers.tokenization_xlm import XLMTokenizer
    ...
    if run_mode == "train":
        ...
        try:
            for epoch in range(args.epochs):
                ...
                printout_allocated_gpu_memory(verbose, "{} epcoh train done".format(model_id))
                model.eval()
                ...
                for (heuristic_test, gold_error, norm_2_noise_eval) in zip(heuritics_zip, gold_error_or_not_zip, norm2noise_zip):
                    ...
                    try:
                        ...
                        loss_test, iter_test, perf_report_test, _ = epoch_run(batchIter_test, tokenizer,
                                                                              ...
                                                                              n_obs_max=n_observation_max_per_epoch_dev_test, verbose=verbose)
