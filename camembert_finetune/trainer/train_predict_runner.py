from camembert.downstream.finetune.env.imports import os, pdb, json

from camembert.downstream.finetune.io_.logger import printing
from camembert.downstream.finetune.evaluate.score.early_stopping_metrics import get_early_stopping_metric
from camembert.downstream.finetune.env.seeds import init_seed
from camembert.downstream.finetune.env.dir.pretrained_model_dir import BERT_MODEL_DIC
from camembert.downstream.finetune.model.constants import NULL_STR
from camembert.downstream.finetune.trainer.run import run
...
def train_predict_eval(args, verbose=1):
    ...
    vocab_size = BERT_MODEL_DIC[args.bert_model]["vocab_size"] if args.bert_model else None
    ...
    printing("INFO : environ is {} so debug set to {}", var=[os.environ.get("ENV", "Unkwnown"), debug],
    ...
    run(args=args, voc_tokenizer=voc_tokenizer, vocab_size=vocab_size, model_dir=model_dir,
        ...
        name_with_epoch=args.name_inflation, encoder=encoder, report=True, verbose=1)
