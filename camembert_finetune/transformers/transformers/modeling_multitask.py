...
from camembert_finetune.env.imports import nn,  OrderedDict, re, pdb, CrossEntropyLoss
from camembert_finetune.model.settings import TASKS_PARAMETER, LABEL_PARAMETER
from camembert_finetune.model.constants import PAD_ID_LOSS_STANDART
from camembert_finetune.env.gpu_tools.gpu_info import printout_allocated_gpu_memory
from camembert_finetune.model.tools import get_key_name_num_label
from camembert_finetune.model.architecture.parser_modules.mlp import MLP
from camembert_finetune.model.architecture.parser_modules.biaffine import Biaffine
...
class BertMultiTask(BertPreTrainedModel):
    ...
    def get_loss(loss_func, label, num_label_dic, labels, logits_dict, task, logit_label, head_label=None):
        if label not in ["heads", "types"]:
            ...
            pred = logits_dict["parsing-types"][(head_label != LABEL_PARAMETER["heads"]["pad_value"]).nonzero()[:, 0], (head_label != LABEL_PARAMETER["heads"]["pad_value"]).nonzero()[:, 1], head_label[head_label != LABEL_PARAMETER["heads"]["pad_value"]]]
